...
const moveUp = () => {
  ...
  for (let col = 0; col < mapWidth; col ++) {
    ...
    for (let row = 0; row < mapHeight; row ++) {
      ...
      if (item.value === 48) {
        // 移动到目标行
        item.targetY = lastMergeValue === 0 ? targetRow : targetRow + 1
        targetRow ++
      } else {
        // 合并
      ...
      for (let row = 0; row + 1 < mapHeight; row++) {
        ...
        if (itemList.length > 1) {
          for (let index = 1; index < itemList.length; index ++) {
            itemList[0].value += itemList[index].value
            // 删除重复的数据
            const targetIndex = mapItems.value.findIndex(item => item === itemList[index])
            if (targetIndex !== -1) {
...
const moveLeft = () => {
  ...
  for (let row = 0; row < mapHeight; row ++) {
    ...
    for (let col = 0; col < mapWidth; col ++) {
      ...
      if (item.value === 48) {
        // 移动到目标行
        item.targetX = lastMergeValue === 0 ? targetCol : targetCol + 1
        targetCol ++
      } else {
        // 与上一个合并
